...
占总时长的 41\%，是工程上最不经济的阶段。为使结果可复核，本文把该数值
对径向网格数、时间步长与水分方程口径的依赖单列为三节：烘干时长随网格
一阶收敛（题目自带的输出网格 $N=20$ 偏低 $1.6\%$），对时间步几乎不敏感
（$\Delta t$ 放大 $24$ 倍只改 $0.17\%$），而改用经典 Fick 口径会使其
系统性缩短 $5.9\%$（至 $57.42$\,h）；该口径差在 $20$ 组数值参数下保持不变，
说明它来自模型口径而非数值实现。径向效应方面，集总（零维）近似会把
$61$\,h 错算成 $11.2$\,h，端面（轴向）效应则不足 $1.1\%$，
...
的全部组合中，经典 Fick 口径相对本文口径稳定在 $-5.9\%$
...
\begin{figure}[htbp]
  ...
  \caption{(a) 关键参数 $\pm20\%$ 扰动对问题三烘干时长的影响
    （基准 60.93\,h，$N=100$、$\Delta t=5$\,s，即表 \ref{tab:grid-N}
...
\begin{itemize}
  \item \textbf{本文口径}（守恒形式）
        $\partial(\rho C)/\partial t=\nabla\!\cdot\!(\rho D\nabla C)$：
        附录 3 的 $\rho=650+128C$ 参与输运，
        累计项系数取 $\rho+C\,\mathrm{d}\rho/\mathrm{d}C$，
        界面通量系数取 $\rho D$，即 $C=2.55$ 处累计系数为 $1302.8$；
  \item \textbf{经典 Fick 口径}
        $\partial C/\partial t=\nabla\!\cdot\!(D\nabla C)$：不含 $\rho$。
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccc}
    \toprule
    $N$ & $h$/mm & \multicolumn{2}{c}{本文守恒口径}
      & \multicolumn{2}{c}{经典 Fick 口径} & 两者相对差 \\
    \cmidrule(lr){3-4}\cmidrule(lr){5-6}
     & & 烘干时长/s & /h & 烘干时长/s & /h & \\
    \midrule
    $20$  & $1.000$ & $216215$ & $60.060$ & $203335$ & $56.482$ & $-5.96\%$ \\
    $50$  & $0.400$ & $218575$ & $60.715$ & $205625$ & $57.118$ & $-5.92\%$ \\
    $100$ & $0.200$ & $219340$ & $60.928$ & $206365$ & $57.324$ & $-5.92\%$ \\
    $200$ & $0.100$ & $219685$ & $61.024$ & $206700$ & $57.417$ & $-5.91\%$ \\
    $400$ & $0.050$ & $219830$ & $61.064$ & $206840$ & $57.456$ & $-5.91\%$ \\
    \midrule
    \multicolumn{2}{c}{相邻网格增量/s}
      & \multicolumn{5}{c}{$+2360,\ +765,\ +345,\ +145$（逐次减半，一阶收敛）} \\
    \bottomrule
  \end{tabular}
\end{table}
...
增加：$N=20\to400$ 依次为 $216215,\,218575,\,219340,\,219685,\,219830$\,s，
相邻增量 $2360,\,765,\,345,\,145$\,s 逐次减半，是一阶收敛的典型形态。
取 $N=200$ 与 $N=400$ 两点作 Richardson 外推，
连续解约为 $219.9\times10^3$\,s（$61.1$\,h），
...
$-5.96\%\sim-5.91\%$，$N=20$ 与 $N=400$ 之间只差 $0.05$ 个百分点。
说明"水分方程用哪种口径"与"网格取多密"是两个相互独立的误差源，
量级上近似可加。

\textbf{（3）已发表结果之间的差异主要来自这两项的组合。}
若同时采用经典 Fick 口径与题目自带的输出网格，
烘干时长为 $56.48$\,h；用经典 Fick 口径但加密到 $N=200$ 为 $57.42$\,h；
用本文口径配输出网格为 $60.06$\,h；本文最终取值 $61.02$\,h。
也就是说，不同论文给出的 $56\sim61$\,h 完全可以由
$\{$口径 $\}\times\{$网格$\}$ 的组合解释，
其中口径贡献 $3.6$\,h（$-5.9\%$）、网格贡献约 $1.0$\,h（$1.6\%$）。

\textbf{（4）本文同时报告三个参照值。} 由于口径差大于网格数值误差，
但小于 $D$ 的经验式不确定度（$\pm20\%$ 对应 $51.9\sim74.5$\,h），
论文在主结果 $61.02$\,h 之外，同时给出 $57.42$\,h（同网格、
经典 Fick 口径）与 $60.06$\,h（同口径、输出网格）两个参照值，
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccc}
    \toprule
    $\Delta t$/s & \multicolumn{2}{c}{本文守恒口径}
      & \multicolumn{2}{c}{经典 Fick 口径} & 两者相对差 & 相对 $\Delta t=5$\,s \\
    \cmidrule(lr){2-3}\cmidrule(lr){4-5}
     & 烘干时长/s & /h & 烘干时长/s & /h & & \\
    \midrule
    $2$   & $219328$ & $60.924$ & $206354$ & $57.321$ & $-5.92\%$ & $-0.005\%$ \\
    $5$   & $219340$ & $60.928$ & $206365$ & $57.324$ & $-5.92\%$ & --- \\
    $10$  & $219360$ & $60.933$ & $206380$ & $57.328$ & $-5.92\%$ & $+0.009\%$ \\
    $20$  & $219400$ & $60.944$ & $206420$ & $57.339$ & $-5.92\%$ & $+0.027\%$ \\
    $60$  & $219540$ & $60.983$ & $206580$ & $57.383$ & $-5.90\%$ & $+0.091\%$ \\
    $120$ & $219720$ & $61.033$ & $206760$ & $57.433$ & $-5.90\%$ & $+0.173\%$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
两种口径的差值只在 $-12974\sim-12960$\,s 之间变化，
波动 $14$\,s，相对差始终为 $-5.9\%$。
...
表面在 $t=14.18$\,h 首次低于 $0.15$\,kg/kg，仅占全程的 $23.2\%$；
其余 $46.8$\,h（$76.8\%$）都是在"外壳已干、只剩湿芯"的状态下度过的。
径向分化最剧烈的时刻出现在 $24$\,h：
中心 $0.2559$ 对表面 $0.0712$，相差 $3.59$ 倍；
...
$D\pm20\%$ 对应 $51.9\sim74.5$\,h，跨度接近一整天。
...
而\textbf{烘干还要再持续 $39$\,h}。问题三固定 $R=2.0$\,cm，
等价于在 $t>22$\,h 的 $39$\,h（占全程 $64\%$）里，
让模型采用的扩散路径比真实药材长 $2.000/1.198-1=67\%$。

更重要的是，附件 2 的收缩时标本身与干壳的形成相互印证：
表面在 $14.18$\,h 达标，而收缩在 $22$\,h 后即基本停止——
外层干燥并刚性化之后，外半径不再变化。
...
$14458/14616$：
\[
  61.02\ \text{h}\ \longrightarrow\ 60.36\ \text{h},
\]
只缩短 $0.66$\,h（$1.1\%$），小于网格误差与口径差。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{2.5cm}p{3.1cm}p{3.9cm}p{4.1cm}}
    ...
    \midrule
    径向分布参数模型 & $Bi$、$Bi_m$
      & $1.39$、$3.24$ & 集总模型 $11.2$\,h，仅分布模型的 $18\%$ \\
    表面与中心分化 & $C$ 之比
      & $24$\,h 时 $3.59$ 倍 & 判据必须由中心控制 \\
    干壳（移动界面） & 表面达标时刻、$D$ 之比
      & $14.18$\,h（$23\%$）、$1/256$ & 后 $77\%$ 时长由干壳控制 \\
    均匀收缩（问题四） & 附件 2 外半径
      & $2.000\to1.198$\,cm，体积 $-64\%$
      & 问题四 $50.31$\,h 对问题三 $61.02$\,h \\
    非均匀径向收缩 & $\varepsilon_r=\beta\left[C_0-C(r,t)\right]$
      & 只有外半径，无法标定 & \textbf{未量化}（建议的改进方向） \\
    端面（轴向）输运 & 轴向项占 $\lambda^2$ 之比
      & $1.0\%\sim1.1\%$ & $-0.66$\,h，可忽略 \\
    \bottomrule
  \end{tabular}
\end{table}

表 \ref{tab:radial} 把"径向"这件事拆成了三层：
\textbf{径向分布本身}是模型的主体（集总近似会把 $61$\,h 错算成 $11$\,h）；
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{2.6cm}p{5.2cm}p{6.2cm}}
    ...
    简化：一维径向（端面） & 二维特征值分析：轴向项仅占 $\lambda^2$ 的
      $1.0\%\sim1.1\%$
      & $-0.66$\,h，可忽略（第 \ref{sec:radial} 节） \\
    简化：均匀收缩 & 附件 2 只给出外半径，径向分布不可标定；
      问题三固定 $R$ 使后 $64\%$ 时长的扩散路径偏长 $67\%$
