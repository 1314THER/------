...
\noindent
干燥是决定中药材成品品质的关键工序之一，其中热风烘干是一种常见的干燥方式。该方式主要包括预热平衡和恒温干燥两个阶段，通过调控烘房温湿环境，完成药材的干燥。在中药材烘干过程中，工艺参数选取不当容易导致干燥效率低、能耗高、成品品质不稳定等问题。而传统的试验优化模式存在成本高、周期长等问题，本文使用数理分析与数值仿真的方法，针对圆柱形药材，建立了径向耦合传热传质的数学模型，并对题目中的四个问题给出了定量解答。

\textbf{针对问题一}，考虑到药材长度与直径之比高达 $6.25$、端面面积仅占侧面面积的 $8\%$，
于是将其简化为无限长圆柱的一维径向轴对称问题。已知热扩散率
$\alpha=1.689\times10^{-7}\,m^2/s$ 与水分扩散系数
$D(C_0)=4.938\times10^{-9}\,m^2/s$ 相差 $34$ 倍，
1800\,s 时热扩散长度 17.4\,mm 已接近半径，然而水分扩散长度只有 $2.98$\,mm，
因此，我们认为该阶段以达热平衡为主。由于 $D$ 只依赖 $C$ 不依赖 $T$、
并且其余物性均为常数，所以\textbf{热方程与水分方程完全解耦}，可独立求解。
本文采用节点中心有限体积法结合后向 Euler 格式与 Picard 不动点迭代法，结果表明：1800\,s 内药材中心含水率始终保持在
2.5500\,kg/kg，而药材表面含水率已降至 1.5103\,kg/kg，水分变化局限在最外侧约为 $3\sim5$\,mm，
温度则在整个截面上重新分布，中心处为 33.5765\,$^\circ$C、表面为 36.7863\,$^\circ$C，
径向温差为 3.21\,$^\circ$C。

\textbf{针对问题二}，由于 $\rho$、$c_p$、$k$ 随 $C$ 变化，
$D$ 也随 $C$ 与 $T$ 变化，两方程由解耦转为强耦合，需要联立迭代。
由烘房最后 1\,h 温度和水分浓度的平均值（49.9989\,$^\circ$C、0.04999\,kg/kg）
确定恒温干燥段工况为 50\,$^\circ$C 与 0.05\,kg/kg。结果表明：药材在约 2.5\,h
内基本达到烘房温度（3\,h 时径向温差仅 0.12\,$^\circ$C），含水率却按照
由表及里的次序下降：0.5\,h 时中心含水率基本保持不变而表面含水率已降至 1.6491，
3\,h 时中心含水率为 $1.7663$、表面含水率为 $1.0082$。

\textbf{针对问题三}，烘干判据“各处含水率低于 0.15\,kg/kg”等价于中心含水率低于 0.15\,kg/kg，
即 $t_{\mathrm{dry}}=\min\{t\mid C(0,t)<0.15\}$。根据题意，本文在问题二、三的模型中并不用考虑收缩，因此没有考虑 $\rho$ 的变化，最终烘干时间经计算为
\textbf{烘干时长 57.42\,h（2.392 天）}。然而，由于题目给出了 $\rho$ 与 $C$ 的关系且二者都在变化，若材料在 $\rho$ 变化的同时保持不收缩，那么 $\rho$ 与 $C$ 的关系需要满足：$\rho=k(1+C)$，否则物质不守恒。然而题目中给出的关系并不满足这个条件。若仅考虑 $\rho$ 的变化不考虑收缩，把体积密度 $\rho$ 写回水分方程（$\partial(\rho C)/\partial t$ 口径）会使其系统性延长 $6.3\%$（至 $61.02$\,h）。
对于整个过程，本文发现，36\,h 后中心处含水率下降非常缓慢，由 $0.1858$ 降至 0.15 耗时长达 21\,h，占总时长的 37\%。本文还研究了该数值
对径向网格数、时间步长的依赖：烘干时长随网格
一阶收敛，对时间步敏感度低。
径向效应方面，端面影响不足 $1.1\%$，一维径向假设成立。

\textbf{针对问题四}，药材半径由 2.000\,cm 收缩至 1.198\,cm，
收缩进程高度前置，
而含水率的下降持续到 50\,h 之后，二者不是简单对应关系，
所以采用实测 $R(t)$ 的方法。引入贴体坐标 $\xi=r/R(t)$ 将动边界转化为固定区域，
方程相应增加对流项 $\xi R\dot R\,\partial C/\partial\xi$；
$R(t)$ 用保单调的三次 Hermite 插值以避免 $\dot R$ 跳变。
经计算得\textbf{烘干时长 50.31\,h（2.096 天）}；
...
中药材烘干以热风烘干为主，分预热平衡与恒温干燥两个阶段；工艺参数选取
不当会造成效率低、能耗高、品质不稳，而传统试验优化成本高、周期长，
需借助数理分析与数值仿真获得干燥规律。
...
问题的物理内核是\emph{含湿多孔介质内的耦合传热传质}：热量由表面向内传导、
...
\begin{enumerate}
  \item[(A1)] \textbf{介质与局部热平衡}：药材视为均质、各向同性的含湿
        多孔介质；任一瞬时、任一位置上水与固体骨架温度相同（局部热平衡），
        故只需一个能量方程，不区分两相温度。固相体积分数允许随干燥收缩
        而变化，但固相自身不可压缩、干物质质量守恒。
  \item[(A2)] \textbf{降维}：忽略轴向输运，问题简化为一维径向轴对称。
        依据是长径比 $L/R=12.5$、两端面面积仅占侧面积的 $2R/L=8\%$，
        且第 \ref{sec:radial} 节按二维特征值给出的端面效应不超过
        $1.1\%$（约 $-0.62$\,h），与本文其余误差相比可忽略。
  \item[(A3)] \textbf{热源与辐射}：药材内部无内热源，忽略辐射换热，
        表面只考虑对流换热与对流传质；烘房空气温度、湿度与流速在药材
        表面均匀分布，$h$ 与 $h_m$ 沿周向和轴向不变。
  \item[(A4)] \textbf{几何}：问题一至三取固定半径 $R=2$\,cm（题面前三问
        未提供收缩数据，也未要求考虑尺寸变化），问题四按附件 2 的实测
        $R(t)$ 收缩。这一取舍不是无依据的简化：其几何依据、忽略收缩的
        误差量级以及把收缩纳入后烘干时长的变化，见第
        \ref{sec:shrink-criterion} 节。
  \item[(A5)] \textbf{物性}：按各问对应的附录取值——问题一用附录 2
        （$\rho$、$c_p$、$k$ 为常数，$D=D(C)$）；问题二、三用附录 3；
        问题四用附录 4。后两者均满足 $D=D(C,T)$、$\rho,c_p,k$ 随 $C$ 变化。
        经验式只在实验区间 $C\in[0,2.55]$ 内使用，不作外推
        （第 \ref{sec:rho} 节）。
  \item[(A6)] \textbf{界面系数}：$h$ 与 $h_m$ 题面未给出，统一沿用附录 2 的
        $h=25$\,W/(m$^2\cdot$K)、$h_m=8\times10^{-7}$\,m/s。这两个系数
        \emph{不能}由附件 1 反求——附件 1 只有空气侧激励、没有药材侧响应，
        参数不可辨识（第 \ref{sec:identify} 节）；其可信度因此改由灵敏度
        分析回答：$h$ 变化 $\pm20\%$ 对烘干时长的影响仅 $\pm0.03\%$、
        $h_m$ 为 $\pm2\sim3\%$（第 \ref{sec:sensitivity} 节）。
  \item[(A7)] \textbf{汽化潜热}：忽略蒸发吸热对表面能量平衡的贡献。
        若按同一密度口径计入 $L_v=2.26\times10^6$\,J/kg，表面温度将被压至
        $6.1\,^\circ$C，低于由空气状态算出的湿球温度 $34.7\,^\circ$C，
        违反热力学第二定律；据此判定题面所给参数体系隐含该项可略，
        依据与能量收支核算见第 \ref{sec:closure} 节。
  \item[(A8)] \textbf{水分输运}：问题一至三用 Fick 型扩散描述，
        表面为第三类边界条件 $-D\left.\partial_rC\right|_{r=R}
        =h_m(C_s-C_\infty)$，且干基密度在方程两侧约去（第
        \ref{sec:rho} 节）；问题四考虑收缩，水分方程改用体积密度的守恒
        形式，并保留坐标变换带来的对流项（第 \ref{sec:p4} 节）。
  \item[(A9)] \textbf{边界数据}：附件 1 的烘房温湿度实测序列
        \textbf{不作平滑、不作差分、不剔除观测点}，直接以 60\,s
        分段线性插值作为边界条件 $T_\infty(t)$、$C_\infty(t)$；
        依据与量化影响见第 \ref{sec:preproc} 节。
  \item[(A10)] \textbf{收缩速率}：附件 2 的半径序列改用保单调三次 Hermite
        插值（PCHIP），以其解析导数 $\dot R$ 作为收缩速率，以避免差分
        放大噪声、以及线性插值使 $\dot R$ 在节点处跳变；理由见第
        \ref{sec:preproc} 节。
\end{enumerate}
...
附件 1 是烘房空气温湿度时序，在模型中充当\emph{边界激励}；附件 2 是药材
半径时序，在问题四中充当\emph{动边界}。二者物理角色不同、预处理要求也不同。
本节先作数据质量诊断，再说明为什么\textbf{附件 1 不作平滑、不作高阶插值、
不作差分，而附件 2 必须作保单调插值}，并量化这些选择的影响。
...
因此附件 1 \emph{不需要}通常意义上的清洗（图 \ref{fig:air}）。
...
\begin{figure}[htbp]
  ...
  \caption{附件 1 实测序列与噪声诊断（$n=241$，间隔 60\,s）。
    (a)(b) 实测序列（实线）与高斯加权局部二次趋势（虚线，窗口 15 点、
    无相位滞后）；(c)(d) 相对趋势的残差，灰带为 $\pm2\sigma$。
    残差呈高频交替型，最大幅值仅约 $2.3\sigma$，无异常点、无缺失。}
  \label{fig:air}
\end{figure}

\subsection{附件 1 的预处理口径：不作平滑、保留线性插值、不作差分}

第一条结论是\textbf{不作平滑}。药材的热时间常数
$R^2/\alpha=0.02^2/1.689\times10^{-7}=2368$\,s 是噪声特征周期
（60--120\,s）的 20--40 倍，平滑所能改变的恰是已被药材滤掉的高频成分；
而附件 1 的升温集中在 $0\sim5400$\,s、仅 90 个采样点，常用的 15 点窗口
（900\,s）已占整个升温段的 $17\%$，会把边界条件整体压低或抬高，影响大于
它想消除的噪声。表 \ref{tab:smooth} 给出四种处理方式对问题一结果的影响
（均取 $N=400$、$\Delta t=1$\,s）：\textbf{水分浓度场几乎不受影响}——
四位小数下最多 $0.4\%$ 的格子发生变化，问题二表 \ref{tab:p2-C} 的 30 个
数字完全不变，问题三、四的烘干时长变化小于 1\,min；\textbf{温度场只在第
4 位小数上变动 $0.014\sim0.055\,^\circ$C}，其中 3 点滑动平均与 5 点
Savitzky--Golay 的偏差与边界噪声不可消除的涨落
$0.116/\sqrt{30}\approx0.02\,^\circ$C 同量级、物理上不可区分，而 15 点窗口的
...
差别只在两点之间（表 \ref{tab:interp}）：边界上 $0.05\sim0.10\,^\circ$C 的
差别进入药材温度场后衰减约 20 倍、只剩 $0.004\sim0.005\,^\circ$C，小于边界
噪声的均值涨落 $0.02\,^\circ$C，用现有数据无法判定哪种插值更准；而自然三次
...
附件 1 只覆盖 $0\sim14400$\,s（4\,h），而整个烘干过程长达 61\,h，故
...
问题四需要收缩速率 $\dot R$ 进入对流项：直接差分会把噪声放大
$1/\Delta t$ 倍，分段线性插值则使 $\dot R$ 在节点处跳变、把间断引入对流项。
因此采用 Fritsch--Carlson 构造的保单调三次 Hermite 插值（PCHIP），它既保证
...
\begin{figure}[htbp]
  ...
  \caption{附件 2 的半径序列与收缩特征（$n=145$，间隔 1800\,s）。
    ...
    峰值 $-0.27$\,cm/h 出现在最初半小时内，说明收缩在干燥初期最剧烈；
    (c) 收缩完成度：$50\%$ 的收缩在 $2.5$\,h 内完成、$99\%$ 在 $22.0$\,h
...
附录给出的物性经验式（$\rho$、$c_p$、$k$）与 $D(C,T)$ 是全部四个问题的数据
基础。其中体积密度 $\rho=a+bC$ 有一个容易被误读的特征：\textbf{斜率远小于
截距}（附录 3 为 $b=128$ 对 $a=650$，附录 4 为 $b=90$ 对 $a=760$，相差
$4\sim8$ 倍）。由干基含水率的定义似应有 $\rho=\rho_d(1+C)$，若 $\rho_d$ 为
常数则斜率应当等于截距——题面两式是否彼此不兼容？\textbf{恒等式本身当然
成立，"斜率等于截距"这一判据却附加了一个未被言明的强假设}，即干燥过程中
物料体积严格不变，而该假设恰在本题中不成立；斜率小于截距正是体积收缩的
定量标志。下文据此反演收缩系数、检验三相体积分数的可容许性，并给出 $\rho$
在水分扩散方程中可以消去的条件——这既是问题一至三忽略收缩的定量依据，
也是问题四必须引入动边界的原因。

\subsubsection{恒等式、隐含假设与收缩系数的定量反演}

记 $m_d$ 为干物质质量、$m_w$ 为水分质量、$V$ 为湿物料总体积，
$\rho_w^0\approx10^3$\,kg/m$^3$ 为纯水真密度。干基含水率、体积密度与湿基
含水率定义为
\begin{equation}
...
\end{equation}
它由定义直接得到，对任何含水物料在任何时刻都成立，无需任何物理假设。
关键在于 $\rho_d$ 是以\textbf{湿物料当前总体积}为基准的\textbf{表观}干物质
密度，而非干物质自身真密度 $\rho_s$（植物干物质约 $1300\sim1600$\,kg/m$^3$），
二者相差一个固相体积分数 $\rho_d=\rho_s\varepsilon_s$；只要 $V$ 随干燥变化，
$\rho_d$ 就不是常数。

把 $\rho=a+bC$ 代入式 \eqref{eq:rho-identity} 得
\begin{equation}
...
\begin{equation}
  ...
  \frac{m_d}{V}\ \text{全程不变}\iff V\ \text{不变（无收缩）}.
  \label{eq:rho-criterion}
\end{equation}
可见"斜率等于截距"并非定义的推论，而是"\textbf{干燥过程中物料体积严格
不变}"这一物理假设的推论。实际物料失水后 $C\downarrow$ 伴随 $V\downarrow$，
$\rho_d=m_d/V\uparrow$，由式 \eqref{eq:rho-d-slope} 得 $b<a$：
\textbf{题面给出的 $b<a$ 不是系数不统一，恰恰是体积收缩的必然标志}；
若给出 $b=a$，那才意味着强行假定了刚性不收缩物料。把经验式代回式
\eqref{eq:rho-d-slope} 还可直接算出表观干基密度：附录 3 的 $\rho_d$ 由
$650$ 降到 $275$\,kg/m$^3$，附录 4 由 $760$ 降到 $279$\,kg/m$^3$，即
\textbf{单位干物质所占体积缩到初始的约 $42\%$}，说明 $\rho_d$ 绝不能当作
材料常数。

为判断 $b$ 的数值是否落在可容许范围内，引入线性收缩模型：设每失去单位体积
的水，物料总体积减少 $\beta$ 个单位体积，$\beta\in[0,1]$ 为\textbf{收缩
系数}（$\beta=0$ 完全不收缩、孔隙全部由空气填充；$\beta=1$ 为理想收缩、
体积减少量恰等于失水体积）。以 $1$\,kg 干物质为基准，
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:shrink-rho}
\end{equation}
这是 $C$ 的双曲函数而非线性函数，说明\textbf{有收缩的物料中 $\rho$ 与 $C$ 的
严格关系不可能是线性的}，题面线性式只能是实验区间 $[0,C_0]$ 上的拟合式。
令式 \eqref{eq:shrink-rho} 在 $C=C_0$ 处与线性式相等，即得端点收缩系数
\begin{equation}
  \beta_{\text{端点}}=\frac{\rho_w^0}{C_0}
  \left[\frac{1+C_0}{a+bC_0}-\frac{1}{a}\right];
  \label{eq:beta-endpoint}
\end{equation}
在 $C\in[0,C_0]$ 上作最小二乘拟合，则得 $\beta_{\mathrm{LSQ}}$ 及其残差。
结果汇总于表 \ref{tab:beta}，拟合曲线见图 \ref{fig:rho-hyp}。
...
两组经验式反演的收缩系数均为 $\beta\approx0.82\sim0.96$，即失水体积的
$80\%\sim96\%$ 转化为物料体积的减少、其余转化为孔隙，这对新鲜植物类物料是
典型且合理的数值（文献中果蔬、中药材的 $\beta$ 普遍在 $0.7\sim1.0$）；
$b=128$ 与 $b=90$ 都稳稳落在 $\beta\in[0,1]$ 所对应的可容许区间内部。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/d26_rho_hypothesis.pdf}
  \caption{不同收缩假设下的 $\rho$–$C$ 关系。实线为题目给定的经验式，
    其余三条为收缩模型式 \eqref{eq:shrink-rho} 在 $\beta=0$（无收缩）、
    $\beta=1$（理想收缩）与反演值 $\beta_{\mathrm{LSQ}}$ 下的曲线；
    水平点线为水的真密度 $1000$\,kg/m$^3$ 与干物质真密度
    $1400$\,kg/m$^3$。(a) 中的五角星标出"若强行要求斜率等于截距"时的
    后果：$C_0=2.55$ 处将得到 $\rho=2307.5$\,kg/m$^3$，超过水与干物质真密度
    所能构成的任何混合物密度，物理上不可能。真正不可能的是"斜率等于截距"
    这一要求本身，而题面经验式与 $\beta\approx0.9$ 的收缩模型高度吻合。}
  \label{fig:rho-hyp}
\end{figure}

\subsubsection{三相体积分数检验与物性式的混合律旁证}

更严格的可容许性判据是水、固、气三相的体积分数处处非负：
\begin{equation}
...
\end{equation}
取 $\rho_s=1400$\,kg/m$^3$ 计算结果如表 \ref{tab:phase}。
...
全区间内三相体积分数均非负，湿态孔隙率约 $9\%\sim10\%$、干态约
$46\%\sim54\%$，与新鲜／干制植物材料的实测值量级一致。与表 \ref{tab:beta}
一起可以判定：\textbf{题面两个密度经验式在数学上自洽、物理上完全可容许}，
它们只是实验区间上的拟合式，使用时不应外推。一个独立的旁证来自 $c_p$ 与
$k$：二者均以湿基含水率 $C/(C+1)=w$ 为自变量，若为质量加权混合律
$c_p=(1-w)c_{p,d}+wc_{p,w}$，则 $w\to1$（纯水）处应外推得到水的物性。
代入附录 3 得
\begin{equation}
  ...
  \label{eq:mixing-law}
\end{equation}
恰为水在常温下的比热容（$4182\sim4186$）与导热系数（$\approx0.60$）；
附录 4 相应为 $4000$\,J/(kg$\cdot$K) 与 $0.32$\,W/(m$\cdot$K)（$k$ 偏低，
对应串联型有效介质模型）。这既\textbf{反向确证了 $C$ 确为干基含水率、
$C/(C+1)$ 确为湿基含水率}，也说明题面物性式经过物理构造：$c_p$、$k$ 用
质量分数混合律（与体积无关，形式严格），而 $\rho$ 用实验线性拟合（与体积
有关，必须引入收缩），三者处理方式的差异本身就自洽。

\subsubsection{对水分扩散方程的影响：$\rho$ 何时可以消去}

设 $\rho_d(\bm x,t)$ 为局部表观干物质密度，则单位体积的水分质量为 $\rho_dC$。
以固相骨架速度 $\bm u$（由收缩引起）为参考，水分守恒与干物质守恒分别为
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:cons-rho}
\end{equation}
由此可明确 $\rho_d$ 的消去条件：\textbf{忽略收缩}时 $\bm u=\bm 0$ 且
$\rho_d$ 为时空常数，两端约去，在圆柱径向一维情形下化为
\begin{equation}
...
\end{equation}
\textbf{考虑收缩}时，只要换用以干物质质量为自变量的 Lagrange 坐标，$\rho_d$
同样被吸收进度规，方程仍化为 $\partial C/\partial t=\partial_\xi(D_\xi
\partial_\xi C)$，其中 $D_\xi=\rho_d^2D$——这正是问题四贴体坐标变换的一维
对应形式。反过来，\textbf{不应把 $\rho=a+bC$ 直接代进扩散方程}：那既无必要，
也会因把表观密度误当材料常数而破坏守恒（第 \ref{sec:drymass} 节给出其后果）。
但在\textbf{能量方程}中 $\rho$ 应当照常使用（按假设 A7 略去蒸发吸热项）：
\begin{equation}
  ...
  \label{eq:energy-rho}
\end{equation}
其中 $\rho c_p$ 是体积热容，与收缩与否无关，$\rho=650+128C$ 直接代入即可。
本文问题二、三采用的正是式 \eqref{eq:rho-eliminated} 的经典 Fick 口径
（敏感性见第 \ref{sec:closure} 节：把 $\rho$ 写回水分方程会使烘干时长
系统性延长 $6.3\%$）。
...
\begin{figure}[htbp]
  ...
  \caption{附录 3、附录 4 的物性随含水率的变化
    ...
    在 $C\to\infty$ 时趋于水的物性；
    (d) 水分扩散系数 $D$ 从初值到判据值下降约 7 倍（若按同一温度比较则
    约 17 倍），\textbf{这正是干燥后期"极慢"的根本原因}——$e^{-0.45/C}$
    一项在低含水率下急剧衰减，$C$ 再降到 $0.05$ 时相对 $0.15$ 又跌去
    $98\%$（见第 \ref{sec:radial} 节的干壳分析）。四张子图共用同一横轴范围，
    便于横向比较。}
  \label{fig:props}
\end{figure}
...
数量级，$\Delta t$ 由 $5$\,s 放宽到 $60$\,s 也仅改变 $0.10\%$。
...
\begin{table}[htbp]
  ...
  \caption{水分方程口径对问题三烘干时长的影响
    ...
    变密度口径为 $61.02$\,h，两种网格给出的相对差同为 $+6.3\%$）}
  \label{tab:clos}
  \small
  \begin{tabular}{lcc}
    \toprule
...
\begin{figure}[htbp]
  ...
  \caption{问题三干燥曲线：中心与表面水分浓度随时间的变化
    ...
    表面自 $12.6$\,h 起即已达标（占全程 $22\%$），全程由中心决定。
...
由于 $\dot R$ 在最初半小时内可达 $-7.6\times10^{-7}$\,m/s
（$0.27$\,cm/h），对流项与扩散项量级相当，
...
它与第 \ref{sec:rho} 节的收缩系数 $\beta$ 一一对应：令双曲式在 $C_0$ 处与
线性式相等，可解出
\begin{equation}
  \beta(r)=\frac{\rho_w^0}{a}\cdot\frac{1-r}{1+rC_0},
  \label{eq:p11-beta}
\end{equation}
代入得 $\beta_3=0.822$、$\beta_4=0.891$，与第 \ref{sec:rho} 节的反演值一致。
二者的分工值得强调：$\beta$ 携带绝对物理含义（失水体积中有多大比例转化为
体积减少），计算它需要密度尺度 $a$ 与水的真密度；而"该不该忽略收缩"是纯几何
决策，只有 $r$ 是必要的。

\subsection{判据一：忽略收缩的闭式阈值}

忽略收缩，就是用固定半径 $R_0$ 近似真实的 $R(C)$，误差用相对线收缩
$s(C;r)=1-R(C)/R_0$ 度量。给定可接受的半径容差 $\varepsilon$ 与工作窗口
$[C_{\min},C_0]$，忽略收缩当且仅当窗口内最大收缩不超过容差，即
$s(C_{\min};r)\le\varepsilon$。由于 $s$ 关于 $r$ 单调递减，该条件等价于
\begin{equation}
  ...
  \label{eq:p11-rstar}
\end{equation}
其中 $n=2$（径向等长）或 $n=3$（各向同性）。反解含水率即得\textbf{临界含水率}
\begin{equation}
...
表 \ref{tab:p11-crit} 的读法：\textbf{预热段}在 5\% 容差下阈值低至 $0.164$
（径向），两套经验式都远高于它，忽略收缩成立；若用更严的 3\% 容差，阈值升到
$0.351$，附录 3 恰好落在临界（$r_3=0.197<0.351$）——所以"预热段线收缩不足
$3\%$"对附录 3 是临界成立、对附录 4 已轻微越界，写成 $3\%\sim4\%$ 更稳妥。
\textbf{问题二窗口}（$C$ 由 $2.55$ 降到 $1.364$）在 5\% 容差下阈值为 $0.507$，
两套经验式已在阈值之下。\textbf{全程窗口}（$C\to0.15$）阈值升到 $0.838$，
两套经验式都远在阈值之下——\textbf{无论用哪套物性，深度干燥段的收缩都不可忽略}。
这正是本文把问题四单列的原因。

需要说明判据的适用范围：$r^\ast$ 与 $C^\ast$ 只依赖 $\varepsilon$ 与工作窗口，
不依赖扩散系数或换热系数，因此是\textbf{先验的}、求解之前就能算出的判据；
但也正因如此，它衡量的是\textbf{几何}误差，回答不了"烘干时长会差多少"。
下一小节补上这一层。

\subsection{判据二：从几何容差到烘干时长}

半径误差不是工程上真正关心的量。为回答"忽略收缩会让烘干时间错多少"，
本文做了一组对照：固定 $a=650$ 与附录 3 的 $c_p$、$k$、$D$，只改斜率 $b$，
对每个 $r$ 分别求固定半径与计入收缩两种解。收缩几何由式 \eqref{eq:p11-ratio}
反演，并用不动点迭代自洽闭合——先用固定半径解出浓度场，按体积平均含水率
反推 $R(t)$，再解一次，直至半径稳定。以
\begin{equation}
...
表 \ref{tab:p11-sweep} 给出三点。

其一，\textbf{几何固定时斜率几乎不影响烘干时长}：$b$ 由 $0$ 增到 $650$，
$t_{\text{固定}}$ 只在 $57.30\sim57.43$\,h 之间变动，全变程 $0.24\%$。
原因是烘干时长由水分扩散控制，而体积密度只以体积热容 $\rho c_p$ 的形式进入
热方程，其影响局限于最初两三小时的升温段；温度一旦追上烘房，$\rho$ 便不再
参与水分方程（第 \ref{sec:rho} 节的消去条件）。所以\textbf{对本文问题二、三
这类固定几何的计算，密度经验式的斜率是无关紧要的参数}。

其二，\textbf{一旦计入收缩，斜率的影响被放大两个数量级}：$r$ 从 $1$ 减到 $0$，
$\Delta$ 从 $0$ 单调增到 $169\%$；附录 3 的 $r=0.197$ 对应 $\Delta=95\%$，
即\textbf{在问题三的物性与窗口下，忽略收缩会把烘干时长高估近一倍}。
这只是判据的后果之一，其数值本身已在下一小节用真实几何复核。

其三，把 $\Delta(r)$ 反解即得\textbf{时长口径的阈值}：要求时长误差不超过
$5\%$、$10\%$、$20\%$，分别需要 $r\ge0.913$、$0.835$、$0.704$。
\textbf{时长判据明显严于几何判据}——同样是 5\% 容差，几何判据给出
$r^\ast=0.838$，时长判据给出 $0.913$。原因很直接：半径以平方进入特征扩散
时间，几何误差会被放大；而题目两套经验式的 $r$ 分别为 $0.197$ 与 $0.118$，
按任一口径都远在阈值之下。
...
而把体积密度写回水分方程只延长 $6.3\%$，$D$ 变化 $\pm20\%$ 带来
$-15\%\sim+22\%$。\textbf{收缩效应比口径效应大近一个数量级}，
也大于参数不确定度。
...
\textbf{总量上互证。}在\textbf{径向等长}口径下，附录 4（$r=0.118$）反推的
绝干半径为 $1.211$\,cm，与附件 2 的实测终半径 $1.198$\,cm 只差 $1.1\%$；
换成收缩系数是 $\beta=0.891$ 对 $0.922$（差 $3.4\%$）。附录 3 的
$1.301$\,cm 则偏 $+8.6\%$。也就是说，题面为问题四同时给出的两组数据
（附件 2 的 $R(t)$ 与附录 4 的 $\rho(C)$）在"干燥全程能缩到多小"这一点上
互相印证，问题四把二者搭配使用是有依据的。
...
\begin{figure}[htbp]
  ...
  \caption{(a) 由附录 3 经验式推算的几何收缩：体积比 $V/V_0$、
    ...
    $R$–$C$ 轨迹的对照：总量上二者相差约 $7\%$，但路径上前者与含水率同步、
    后者高度前置。}
...
\begin{enumerate}
  \item \textbf{问题一（预热段，$C$ 由 $2.55$ 降到约 $2.0$）}：窗口末端含水率
        高于 $C^\ast$（5\% 容差下附录 3 为 $1.95$、附录 4 为 $2.06$），
        几何误差不足 $5\%$，忽略收缩严格成立。本文取固定半径与经典 Fick 口径。
...
\begin{enumerate}
  \item \textbf{单参数约化}：收缩几何只由斜率截距比 $r=b/a$ 决定，
        截距与系数的绝对大小都不进入几何；$r$ 与第 \ref{sec:rho} 节的
        收缩系数 $\beta$ 一一对应，但只有 $r$ 是纯几何决策所必需的。
  \item \textbf{几何判据}：忽略收缩安全 $\iff r\ge r^\ast(\varepsilon,C_{\min})$；
        对应的临界含水率为 $C^\ast(\varepsilon,r)$。两者都是闭式，
        求解之前即可判定。题面两式的 $r$ 在预热窗口内安全、在全程窗口内
        远低于阈值。
  \item \textbf{时长判据}：把判据从半径容差升级为烘干时长误差，
        $\Delta\le5\%/10\%/20\%$ 分别要求 $r\ge0.913/0.835/0.704$，
        比几何判据更严（半径以平方进入特征扩散时间）。
  \item \textbf{代价排序}：在同一物性体系下，收缩对烘干时长的影响
        （$-49\%\sim-58\%$）比水分方程口径（$+6.3\%$）大近一个数量级，
        也大于 $D$ 的 $\pm20\%$ 不确定度；而固定几何时斜率本身几乎不影响
        时长（全变程 $0.24\%$）。
  \item \textbf{数据一致性}：由 $r$ 反推的绝干半径与附件 2 实测终半径相差
        $1.1\%$，说明判据所在的物性体系与实测收缩在总量上互证；
        两者在路径上相差 $10.6\%\sim22.4\%$，这正是问题四取实测 $R(t)$
        的理由。
\end{enumerate}
