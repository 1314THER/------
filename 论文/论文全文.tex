...
\begin{enumerate}
  ...
  \item[(A2)] \textbf{降维}：忽略轴向输运，问题简化为一维径向轴对称。
        ...
        （误差分析见 \ref{sec:radial} ）
  \item[(A3)] \textbf{热源与辐射}：只考虑对流换热与对流传质；
...
其中体积密度 $\rho=a+bC$ 有一个容易引起误解的特征：附录 3 中
$a=650,\ b=128$，附录 4 中 $a=760,\ b=90$，均有 $b<a$。若简单把
$\rho=\rho_d(1+C)$ 中的 $\rho_d$ 视为常数，似乎会得到``斜率应等于截距''，
进而怀疑题面经验式不自洽。实际上，$\rho_d$ 是以当前湿物料总体积为基准的
表观干物质密度，只要干燥过程中存在体积变化，它就不是常数。

因此，本节只回答两个基础问题：\textbf{题面物性式是否具有物理可容许性，以及
$\rho$ 应如何进入守恒方程}。至于如何进一步由 $\rho=a+bC$ 提取收缩信息、
建立``何时可以忽略收缩''的先验判据，并量化其对烘干时长的影响，统一放在
第 11 节讨论，以避免重复推导并突出该判据的独立性。

\subsubsection{密度经验式的物理含义}

记 $m_d$ 为干物质质量、$m_w$ 为水分质量、$V$ 为湿物料当前总体积，
$\rho_w^0\approx10^3$\,kg/m$^3$ 为纯水真密度。干基含水率、体积密度与湿基
含水率定义为
\begin{equation}
  C=\frac{m_w}{m_d},\qquad
  \rho=\frac{m_d+m_w}{V},\qquad
...
\end{equation}
这里 $\rho_d$ 是以当前总体积 $V$ 为基准的\textbf{表观干物质密度}，
而不是与几何无关的材料真密度。只要 $V$ 随干燥过程变化，$\rho_d$ 就会随之变化。

将题面的线性经验式 $\rho=a+bC$ 代入式 \eqref{eq:rho-identity}，有
\begin{equation}
  \rho_d(C)=\frac{a+bC}{1+C},\qquad
  \frac{\mathrm{d}\rho_d}{\mathrm{d}C}
  =\frac{b-a}{(1+C)^2}.
...
\end{equation}
因此，``斜率等于截距''并不是干基含水率定义本身的要求，而是额外加入
``总体积始终不变''之后才得到的结果。题面两组经验式均满足 $b<a$，
意味着随 $C$ 降低，$\rho_d=m_d/V$ 增大，即单位干物质所占总体积减小；
这与干燥收缩的物理图像一致，而不是经验式之间的矛盾。

例如，在 $C_0=2.55$ 时，附录 3、4 对应的表观干物质密度分别约为
$275$ 和 $279$\,kg/m$^3$；当 $C\to0$ 时分别趋于 $650$ 和
$760$\,kg/m$^3$。这一变化已经说明题面密度式隐含明显的体积变化信息。
\textbf{如何把这种信息进一步转化为收缩量、无量纲判据及烘干时长误差，
留至第 11 节统一处理。}

\subsubsection{三相体积分数检验与物性式的混合律旁证}

进一步检验题面密度式的物理可容许性。水、固、气三相体积分数分别为
\begin{equation}
  \varepsilon_w=\frac{\rho\,w}{\rho_w^0},\qquad
  \varepsilon_s=\frac{\rho\,(1-w)}{\rho_s},\qquad
  \varepsilon_a=1-\varepsilon_w-\varepsilon_s .
  \label{eq:three-phase}
\end{equation}
取 $\rho_s=1400$\,kg/m$^3$，在 $C\in[0,2.55]$ 内逐点计算，
三相体积分数均保持非负，代表性端点见表 \ref{tab:phase}。
...
可见两组经验式在整个使用区间内均满足三相体积分数非负，湿态孔隙率约为
$9\%\sim10\%$，干态约为 $46\%\sim54\%$，不存在体积分数越界等明显的
物理矛盾。因此，题面的线性密度关系应理解为实验区间内的有效拟合式，而不应
按``固定体积下的理论恒等式''解释，更不宜超出给定含水率区间外推。

另一个独立旁证来自 $c_p$ 与 $k$。二者均以湿基含水率
\[
  w=\frac{C}{1+C}
\]
为自变量。若 $c_p$ 采用质量加权混合形式
$c_p=(1-w)c_{p,d}+wc_{p,w}$，则在 $w\to1$ 时应趋近水的物性。
代入附录 3 得
\begin{equation}
  c_p\big|_{w=1}=1450+2736
  =4186\ \mathrm{J/(kg\cdot K)},\qquad
  k\big|_{w=1}=0.21+0.38
  =0.59\ \mathrm{W/(m\cdot K)},
  \label{eq:mixing-law}
\end{equation}
与水在常温下的比热容和导热系数量级一致。由此也可反向确认题目中的
$C$ 确按干基含水率定义，而 $C/(1+C)$ 为相应湿基含水率。总体上，
$c_p$、$k$ 采用组成相关的有效物性关系，$\rho$ 则同时包含体积变化信息，
三者的形式并不存在内在冲突。
...
速度，则单位体积水分质量为 $\rho_dC$。水分守恒与干物质守恒分别写为
\begin{equation}
...
闭合口径}，而不能仅凭题面给出的 $\rho(C)$ 决定。问题一至三采用固定几何的
经典 Fick 口径，即不显式描述固相骨架运动，取 $\bm u=\bm 0$，并以固定的
干物质参考量定义含水率。在这一口径下 $\rho_d$ 在方程两侧约去，得到
\begin{equation}
  \frac{\partial C}{\partial t}
...
一旦显式考虑收缩，$\bm u\neq\bm 0$ 且计算区域随时间变化，此时必须把骨架运动
与移动边界同时纳入守恒关系，再通过贴体坐标将动区域映射到固定区域。
因此，\textbf{不能在保持固定半径的同时，又把题面的体积密度
$\rho=a+bC$ 直接写入水分累积项}；这样会混合两套不同的闭合假设，并导致
干物质守恒失配。第 \ref{sec:drymass} 节将对此给出数值检验，第 11 节则从
收缩几何与烘干时长两个层面进一步量化其影响。

与水分方程不同，题目给出的 $\rho(C)$ 仍应按经验物性用于能量方程，
因为其与 $c_p(C)$ 共同构成体积热容：
\begin{equation}
  ...
  \label{eq:energy-rho}
\end{equation}
因此，``$\rho$ 不直接写入问题二、三的水分扩散方程''并不意味着忽略题面密度
数据，而是因为\textbf{质量守恒与能量守恒中 $\rho$ 的物理角色不同}。
本文问题二、三采用式 \eqref{eq:rho-eliminated} 的经典 Fick 口径，而问题四
在移动边界下重新建立守恒方程。作为口径敏感性检验，若把体积密度直接写回
固定几何的水分方程，所得烘干时长会系统性延长约 $6.3\%$
（第 \ref{sec:closure} 节）。
...
整个截面上基本重新分布，而含水率仅在表层发生变化}。这恰好是预热平衡阶段的意义所在
...
内部扩散成为限制环节。三个参数的灵敏度对比见表 \ref{tab:sens}，
两种水分方程口径的对比见图 \ref{fig:closures}。
...
\textbf{（3）加密网格不能消掉调和平均的偏差。} 把两种取法放在同一组网格上
比较：调和平均在 $N=100$ 时要 $71.57$\,h
（$2.98$ 天，已跌出题面所述的 $2\sim3$ 天区间），$N=200$ 降到 $58.96$\,h，
$N=400$ 仍为 $57.69$\,h（表 \ref{tab:face-mean} 给出 $N=200$ 的三种取法
对比）、比同网格的算术平均高 $0.41\%$；而算术平均在
