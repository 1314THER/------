...
而含水率的缓慢下降持续到 40\,h 之后，说明二者不存在简单对应关系，
故直接采用实测 $R(t)$。引入贴体坐标 $\xi=r/R(t)$ 将动边界化为固定区域，
$\xi$ 是随材料均匀收缩的物质坐标，故不产生对流项；水分方程改用干基密度
$\rho_d=\rho/(1+C)$ 的变密度守恒形式（收缩与密度在物理上相互绑定）。
求得\textbf{烘干时长 39.55\,h（1.648 天）}，比不考虑收缩时缩短 31.1\%；
作为对照，同一物性下固定半径的烘干时长为 4.12 天，可见
\textbf{忽视收缩会将烘干时长高估约 1.5 倍}。
...
因需要连续的 $R(t)$ 进入 $R^2$ 因子与表面边界，采用保单调三次 Hermite 插值。
...
\begin{enumerate}
  ...
  \item[(A10)] 附件 2 的半径序列改用保单调三次 Hermite 插值（PCHIP），
        得到连续的 $R(t)$，理由见第 \ref{sec:preproc} 节。
\end{enumerate}
...
问题四需要连续的 $R(t)$ 进入 $R^2$ 因子与表面边界。直接线性插值虽连续，
但会使 $R(t)$ 在节点处出现折角，令 $R^2$ 因子与边界系数不光滑。因此本文
采用 Fritsch--Carlson 构造的保单调三次 Hermite 插值（PCHIP），它既保证
$C^1$ 连续，又保持数据单调、不产生过冲。数值检验表明插值后 $R(t)$ 单调
递减、未出现插值引起的非物理膨胀；三次样条在该序列上会出现过冲，故不采用。
...
\end{equation}
其中 $\dot R=\mathrm{d}R/\mathrm{d}t<0$。$\xi$ 是随材料\textbf{均匀收缩}的
物质坐标，$C$、$T$ 都是随材料运动的量，其材料导数即
$\left.\partial/\partial t\right|_{\xi}$，故坐标变换\textbf{不产生对流项}。
代入式 \eqref{eq:p2-heat} 并整理，得
\begin{equation}
  ...
  \label{eq:p4-heat}
\end{equation}
水分方程改用干基密度 $\rho_d=\rho/(1+C)$ 的变密度守恒形式
\begin{equation}
  \left(\rho_d+C\frac{\mathrm{d}\rho_d}{\mathrm{d}C}\right)\xi R^2
   \frac{\partial C}{\partial t}
  =\frac{\partial}{\partial\xi}
   \left(\rho_d D\,\xi\frac{\partial C}{\partial\xi}\right),
  \label{eq:p4-mass}
\end{equation}
其中 $\rho_d=\rho/(1+C)$ 是单位体积内的干料质量：收缩使干料更密实，
$\rho_d$ 随 $C$ 与 $R(t)$ 共同变化，因此问题四\textbf{必须保留密度}——
这正是它与问题二、三的关键区别（问题二、三骨架刚性、$\rho_d$ 为常数、
在水分方程两侧约去）。收缩只通过式中的 $R^2$ 因子（扩散路径缩短）起作用；
若令 $R$ 恒定且 $\rho_d$ 为常数，式 \eqref{eq:p4-heat}、
\eqref{eq:p4-mass} 退化为问题二的方程组。
...
\begin{equation}
  ...
  \label{eq:p4-bc}
\end{equation}
水分边界两侧的干基密度 $\rho_d$ 相消，故形式与问题二一致。

\subsubsection{$R(t)$ 的处理}

附件 2 为每 $1800$\,s 一个点的离散数据。本文采用
\textbf{单调三次 Hermite 插值（PCHIP，Fritsch--Carlson 构造）}
得到连续的 $R(t)$，它保证 $C^1$ 连续、保持数据的单调性、不产生过冲，
使 $R^2$ 因子与表面边界中的 $R$ 光滑演化。
收缩终止的时刻由数据本身决定，无需外推。
...
\end{equation}
水分方程的累计项按链式法则以
$\rho_d+C\,\mathrm{d}\rho_d/\mathrm{d}C$ 滞后处理，界面上的
$\rho_d D$ 与 $k$ 由两侧节点值按算术平均构造。时间方向与非线性处理同问题二，
取 $N=200$、$\Delta t=5$\,s。
...
模型给出的烘干时长为
\begin{equation}
  t_{\mathrm{dry}}=142370\ \mathrm{s}=39.55\ \mathrm{h}=1.648\ \text{天},
\end{equation}
比不考虑收缩的问题三（$57.42$\,h）缩短了约 $31.1\%$。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccc}
    ...
    \cmidrule(lr){2-6}
     & 0     & 0.5   & 1.0   & 1.5   & 2.0 & \\
    \midrule
    6  & 0.9171 & 0.8185 & 0.5422 & —— & —— & 0.1797 \\
    12 & 0.3929 & 0.3560 & 0.2390 & —— & —— & 0.0818 \\
    18 & 0.2615 & 0.2405 & 0.1682 & —— & —— & 0.0629 \\
    24 & 0.2068 & 0.1920 & 0.1389 & —— & —— & 0.0569 \\
    30 & 0.1770 & 0.1655 & 0.1228 & —— & —— & 0.0544 \\
    36 & 0.1582 & 0.1486 & 0.1124 & —— & —— & 0.0530 \\
    烘干结束 & 0.1500 & 0.1412 & 0.1078 & —— & —— & 0.0525 \\
    \bottomrule
  \end{tabular}
\end{table}
...
得烘干时长 $355575$\,s $=4.12$ 天；计入收缩后为 $1.65$ 天，
缩短为原来的 $40.0\%$。

这一效应可从三个层面理解：扩散路程由 $R$ 缩短至 $0.6R$，
特征扩散时间按 $R^2$ 计减少到 $36\%$；
单位体积的表面积按 $1/R$ 计增大约 $1.67$ 倍，
表面交换能力相对增强；此外收缩使干基密度 $\rho_d=\rho/(1+C)$ 增大、
变密度守恒形式下的有效扩散系数 $\rho_d D/(\rho_d+C\rho_d')$ 随之增大，
进一步加速了内部水分的再分布。

这一结论有明确的工程含义：\textbf{忽视收缩会严重高估烘干时长}
（本例中高估约 $1.5$ 倍）。若按不考虑收缩的结果安排生产，
...
\begin{figure}[htbp]
  ...
  \caption{问题四：收缩历程与烘干时长的对照
    ...
    与保单调三次 Hermite 插值曲线；插值后 $R$ 单调递减、
    无过冲。(b) 中心水分浓度随时间的变化：含收缩时 39.55\,h 达标，
    固定半径的对照工况需 98.77\,h，收缩使烘干时长缩短约 60\%。
    两图均为确定性数值结果，不含统计误差。}
  \label{fig:p4}
\end{figure}
...
\begin{enumerate}
  \item \textbf{退化检验}：把附录 4 物性替换为附录 2 的常数物性
        （$\rho_d$ 为常数）并令 $R$ 恒定，式 \eqref{eq:p4-heat}、
        \eqref{eq:p4-mass} 退化为问题一模型，程序实测退化解与问题一解
        完全一致，说明贴体坐标与变密度离散格式的实现正确。
  \item \textbf{单调性检验}：PCHIP 插值后的 $R(t)$ 单调递减，
        未出现插值过冲引起的非物理膨胀。
  \item \textbf{合理性检验}：收缩使烘干时长缩短，
        与"扩散路径变短有利于干燥"的物理预期一致；
        问题三 $57.42$\,h 落在题面所述 $2\sim3$ 天区间内，
        问题四因收缩缩短至 $39.55$\,h（$1.65$ 天），同样处于合理量级。
\end{enumerate}
...
问题四在贴体坐标 $\xi=r/R(t)$ 下求解。$\xi$ 是随材料均匀收缩的物质坐标，
故方程\textbf{不含对流项}；水分方程用干基密度 $\rho_d=\rho/(1+C)$ 的
变密度守恒形式，$R(t)$ 由附件 2 的 PCHIP 插值给出
（理由见第 \ref{sec:preproc} 节）。
