...
\begin{figure}[htbp]
  ...
  \caption{附录 \ref{app:rho} 蒸发潜热否决的定量依据：按题给 $h_m$
    估算的潜热需求远大于对流供热能力，比值全程大于 $1$，强行计入会把
    表面温度压到湿球温度以下。}
  \label{fig:energy}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{问题一若干时刻的径向剖面族：(a) 温度剖面，$1800$\,s 时
    径向温差 $3.24\,^\circ$C；(b) 水分浓度剖面，梯度集中在表面附近。}
...
（剖面族见图 \ref{fig:p1-profiles}）。这说明过程始终由内部扩散控制。
...
按竞赛规范，支撑材料为一个压缩包，其文件列表如下；其中与建模直接相关的
求解、对照、灵敏度与校验程序全文列于本节之后。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{4.6cm}p{9.4cm}}
    ...
    \texttt{代码/solve\_p234.py} & 问题二、三、四主求解器（变物性、动边界） \\
    ...
    \texttt{代码/prepare\_outputs.py} & 由数值解生成 \texttt{result*.xlsx} \\
    \texttt{代码/paper\_tables.py} & 由数值解生成论文各表 \\
    \texttt{代码/verify\_paper\_numbers.py} & 论文 347 项数字的自动核对 \\
    \texttt{代码/prepare\_figure\_data.py}、\texttt{代码/make\_figures.py}
      & 插图数据准备与绘制（含 \texttt{figlib/} 样式库） \\
    \texttt{代码/build\_result1.mjs}、\texttt{build\_results234.mjs}
      & 结果表的 JavaScript 生成脚本 \\
    \texttt{输出/} & 全部数值结果、图像数据与审计文件
      （\texttt{result1--4.xlsx}、\texttt{*.npz}、\texttt{figdata/} 等） \\
    \texttt{建模/}、\texttt{思考-斜率截距与收缩/}
      & 建模推导、收缩判据的独立复核脚本与报告 \\
    \texttt{论文/figs/} & 正文与附录插图的 PDF/PNG 源文件 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\texttt{verify\_paper\_numbers.py}。
