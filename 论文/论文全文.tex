...
\subsection{物性关联式的自洽性检验与收缩效应的定量反演}
\label{sec:rho}
...
\subsubsection{恒等式与"斜率等于截距"的隐含假设}
...
\subsubsection{旁证：$c_p$ 与 $k$ 经验式的混合律结构}
...
\subsubsection{对水分扩散方程的影响：$\rho$ 何时可以消去}
...
\begin{figure}[htbp]
  ...
  \caption{干物质质量的守恒性检验：按式
    ...
    右：问题四（含收缩、附录 4）先降到 $-27\%$ 左右，再随半径停止收缩
    回升到 $-10.9\%$（未平滑的原始曲线因半径越过 $0.1$\,cm 网格线而带锯齿）。
...
\subsubsection{必须用径向分布参数模型：两个 Biot 数}
