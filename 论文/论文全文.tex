...
并在线性插值、PCHIP 与三次样条中保留不产生过冲的分段线性插值；附件 2 的
半径序列则用保单调三次 Hermite 插值，获得不过冲、$C^1$ 连续的收缩几何。
...
\textbf{烘干时长 51.08\,h（2.128 天）}；同物性、同口径下固定半径需 $129.81$\,h，
即\textbf{忽略收缩会把烘干时长高估为 2.54 倍（154\%）}。

全部结论经过网格、时间步、界面取法、水分口径、参数灵敏度与干物质守恒六类
自检，误差来源可排序、结论可复核：$D$ 的不确定度（$\pm20\%$ 对应
$48.9\sim70.0$\,h）$>$ 水分方程口径（$+6.3\%/-15.6\%$）$>$ 网格（$1.6\%$）
$>$ 端面效应（不超过 $1.1\%$）$>$ 时间步（$0.2\%$）$>$ 表面换热系数
（$0.03\%$）。
...
会使进入扩散算子与边界条件的几何量出现阶梯。因此采用 Fritsch--Carlson
...
传质系数计入后表面温度会被压到湿球温度以下、与物理不相容，故本文按假设 A7
...
不进入水分方程。具体误差分析见 §\ref{sec:closure}。
...
$275$ 升至 $582$\,kg/m$^3$（至判据 $C=0.15$；$\rho_d\equiv\rho/(1+C)$ 的
定义见第 \ref{sec:rho} 节）；而问题二、三又要求
...
$R(t)^2$、表面边界条件多一个 $R$；水分方程仍是经典 Fick 口径，体积密度
$\rho$ 不进入（理由见第 \ref{sec:closure} 节）。这样四个问题共用同一个
物质输运口径，问题四与问题三的差异\emph{只有几何}一个因素，二者的比较
才是严格单因素的。若令 $\dot R\equiv0$，式 \eqref{eq:p4-heat}、
\eqref{eq:p4-mass} 严格退化为问题二的方程组。
...
（PCHIP，Fritsch--Carlson 构造）}，它既保证 $C^1$ 连续，又保持数据的
单调性、不产生过冲。
收缩终止的时刻由数据本身决定，无需外推。
...
\begin{figure}[htbp]
  ...
  \caption{问题四：收缩历程与烘干时长的对照
    ...
    固定半径的对照工况需 129.81\,h，收缩使烘干时长缩短约 61\%。
    两图均为确定性数值结果，不含统计误差。}
  \label{fig:p4}
\end{figure}
...
$1.6\%$ 与 $0.2\%$，合计不超过 $1.8\%$，
仍小于口径差（$6.3\%\sim15.6\%$）与参数不确定度（$-15\%\sim+22\%$）。
...
它与第 \ref{sec:radial} 节标定的收缩系数 $\beta$ 一一对应：令双曲式在 $C_0$ 处与
线性式相等，可解出
\begin{equation}
  \beta(r)=\frac{\rho_w^0}{a}\cdot\frac{1-r}{1+rC_0},
  \label{eq:p11-beta}
\end{equation}
代入得 $\beta_3=0.822$、$\beta_4=0.891$，与由附件 2 实测外半径反推的
$\beta=0.922$ 相差 $3.4\%$（第 \ref{sec:p11-crosscheck} 节）。
...
$1.198$\,cm 只差 $1.1\%$；换成收缩系数是 $\beta=0.891$ 对 $0.922$
（差 $3.4\%$）。附录 3 的 $1.301$\,cm 则偏 $+8.6\%$。需要说明的是，
...
原因是 $\bar D\sim10^{-9}$\,m$^2$/s 时 $61$\,h 内的轴向扩散长度仅
...
半径，可标定唯一的收缩系数 $\beta=0.922$（用于检验密度式隐含的收缩总量，
见第 \ref{sec:p11-crosscheck} 节），但无法标定径向分布；该项如实标注为
\textbf{未量化}，也是本文建议的首要改进方向。
...
经典 Fick 口径（不显式描述骨架运动，$\bm u=\bm 0$），$\rho_d$ 在方程两侧
约去，
\begin{equation}
  \frac{\partial C}{\partial t}
  =\frac{1}{r}\frac{\partial}{\partial r}
...
\end{equation}
一旦显式考虑收缩，$\bm u\neq\bm 0$ 且计算区域随时间变化，必须把骨架运动与
移动边界同时纳入守恒关系，再通过贴体坐标把动区域映射到固定区域。因此，
\textbf{不能在保持固定半径的同时，又把题面的体积密度 $\rho=a+bC$ 直接写入
水分累积项}，否则会混合两套闭合假设并导致干物质守恒失配（数值检验见
第 \ref{sec:drymass} 节）。作为口径敏感性检验，若把体积密度直接写回固定
几何的水分方程，所得烘干时长会系统性延长约 $6.3\%$（表 \ref{tab:clos}）；
改按干基密度守恒口径则缩短 $15.6\%$，两者都在正文第 \ref{sec:closure} 节
一并报告。
...
\]
由附件 2 标定唯一的收缩系数 $\beta=0.922$。本文问题四只用单一 $R(t)$，
相当于上式在径向上取平均；把 $\beta$ 的径向差异显式建模，是本题最有价值的
一步改进，但附件 2 只记录了外半径，无法直接标定径向分布，故本文如实把该项
标注为\textbf{未量化}。
...
详细使用情况见支撑材料；模型建立、公式推导、数值方案与全部结论均由参赛队
独立完成并逐项复核。
