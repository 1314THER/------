...
附录 3 的 $D$ 是针对该干基含水率拟合的有效扩散系数，密度效应已被吸收进
$D$；严格从水量守恒出发，干料体积密度 $\rho_d=\rho/(1+C)$ 在刚性骨架下
为常数、两侧约去，故体积密度 $\rho$ 只以 $\rho c_p$ 进入热方程、
不进入水分方程。
...
仍采用节点中心有限体积法，但此时每个控制体的"热容"（$\rho c_p$）随解变化。
水分方程左端为 $\partial C/\partial t$；界面上的 $D$ 与 $k$ 由两侧节点值
按算术平均构造。
...
\end{equation}
其中 $\dot R=\mathrm{d}R/\mathrm{d}t<0$。$\xi$ 是随材料均匀收缩的物质坐标，
$C$、$T$ 的材料导数即 $\left.\partial/\partial t\right|_{\xi}$，故坐标变换
不产生对流项。代入式 \eqref{eq:p2-heat} 并整理，得
\begin{equation}
...
$\rho_d$ 随 $C$ 与 $R(t)$ 共同变化，因此问题四必须保留密度；收缩只通过
$R^2$ 因子起作用。若令 $R$ 恒定且 $\rho_d$ 为常数，上两式退化为问题二的方程组。
...
得到连续的 $R(t)$，它保证 $C^1$ 连续、保持单调性、不产生过冲，
...
\end{equation}
水分方程的累计项按链式法则以
$\rho_d+C\,\mathrm{d}\rho_d/\mathrm{d}C$ 滞后处理。时间方向与非线性处理同问题二，
取 $N=200$、$\Delta t=5$\,s。
...
表面交换能力相对增强；此外收缩使干基密度 $\rho_d$ 增大、
变密度守恒形式下的有效扩散系数增大，进一步加速内部水分的再分布。
...
\begin{enumerate}
  ...
  \item \textbf{合理性检验}：收缩使烘干时长缩短，
        ...
        问题三 $57.42$\,h、问题四 $39.55$\,h 均落在题面所述 $2\sim3$ 天区间内。
\end{enumerate}
